...
func Deploy(ctx context.Context, configFilename string) error {
	...
	opts := dockerOptions{image: config.Image, repo: config.Repo, baseImage: config.BaseImage, buildTool: config.BuildTool, existingImage: config.ExistingImage}
...
github.com/eberkley/weaver module version %s (internal version %s).
...
updating the 'weaver-kube' command by running the following.
	...
	go install github.com/eberkley/weaver-kube/cmd/weaver-kube@latest
